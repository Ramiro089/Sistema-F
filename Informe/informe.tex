...
%Viendo primero la parte de declaracion de tipo, podemos ver que esta posee 3 elementos de mas 
%que en la gramatica del Sistema F, estos son BoundForAll, ListEmpty y ForAllLambd, donde sus funcioens son:
...
\begin{itemize}[label=$\bullet$]
  ...
  %\item {\textbf{ForAllLambd}\\
  %La función que tiene el ForAllLambd es la misma que la del ForAllT. En un inicio dicha no existía ForAllLambd solo ForAllT (els segundo asi la funciones del primero),  
  %pero esto acarriaba un problema a la hora de realizar el printType. Veamos este problema, se puede ingresar algo asi:
  
  %%El printType no pude distinguir que variable cuantificada va en cada '/\textbackslash'. Una solución seria un contador, el cual va contando la cantidad de veces que 
  %%aparece el '/\textbackslash', que en la representación seria un ForAllT. 
  %%Pero si se tiene este otro ejemplo:
  
  %el '\textbackslash /' también esta representado por un ForAllT, entonces el contador incrementaría erróneamente. Ademas que es imposible distinguir cuando el ForAllT 
  %se refiere a un '\textbackslash /' o a un '/\textbackslash'.
  %Entonces para evitar este problema se define un nuevo constructor ForAllLambda que representa al '/\textbackslash', minetras que el ForAllT representaria el 
  %'\textbackslash /'. 
  %
  %Pero debido a esto se tiene que definir 'manualmente' el Eq ya que se necesita que el ForAllLambd y el ForAllT sean lo mismo, debido que cuando se aria una sustitucion 
  %como por ejemplo: 
  %\begin{center}
  %  (\textbackslash b:(\textbackslash /B. B -$>$ B) . b) (/\textbackslash X. \textbackslash x:X. x)
  %\end{center}  
  %el tipo de b usa ForAllT y el tipo que lo sustituye usa ForAllLambd.
  %}
